...
def add_user_to_group(group_code, user_id, user_name):
    ...
    if not row:
        ...
        cur.execute("""
        UPDATE group_users SET user_name=? WHERE group_code=? AND user_id=?
...
def get_user_active_group(user_id):
    ...
    cur = conn.cursor()
    cur.execute("""
    SELECT group_code FROM group_users
...
def save_movie_rating(user_id, movie_id, rating):
    ...
    cur = conn.cursor()
    cur.execute("""
    SELECT id FROM ratings WHERE group_code=? AND user_id=? AND movie_id=?
...
def get_common_movies_for_group(group_code):
    ...
    GROUP BY movie_id HAVING COUNT(DISTINCT user_id)=?
    """
...
# Функции для фильтров (только тип контента)

def update_group_filters(group_code, content_type):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
    UPDATE groups
    SET filter_genres=NULL,
        filter_year_start=NULL,
        filter_year_end=NULL,
        filter_type=?
    WHERE code=?
    """, (content_type, group_code))
    conn.commit()
    conn.close()
...
def get_group_filters(group_code):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("SELECT filter_type FROM groups WHERE code=?",(group_code,))
    row = cur.fetchone()
    conn.close()
