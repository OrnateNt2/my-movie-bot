...
async def remove_user_cb(call: CallbackQuery):
    ...
    from bot.database.models import get_user_active_group, get_group_creator_id, leave_group
    ...
    if get_group_creator_id(group_code) != admin_id:
        await call.answer("Вы не создатель группы.")
        return
    ok = leave_group(target_user_id, group_code)
    if ok:
        await call.answer("Участник исключён!")
        await call.message.edit_reply_markup()
        await call.message.answer("Список участников обновлён.")
    else:
