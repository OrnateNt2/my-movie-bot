...
async def process_menu(message: types.Message, state):
    text = message.text
    if text == "Создать группу":
        ...
        if not group_code:
            await message.answer("Нет активной группы. Создайте/присоединитесь и сделайте её активной через Мои группы.")
            return
        groups = get_common_movies_for_group(group_code)
        if not groups:
