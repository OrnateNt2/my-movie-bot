...
from bot.services.kinopoisk_api import get_random_movie_random, get_movie_info_by_id
...
async def show_movie_by_user(bot, user_id):
    ...
    if not group_code:
        ...
        return
    (content_type, rating_min, rating_max, year_start, year_end) = get_group_filters(group_code)
    movie_data = get_random_movie_random(content_type=content_type, rating_min=rating_min, rating_max=rating_max, year_start=year_start, year_end=year_end)
    if not movie_data:
